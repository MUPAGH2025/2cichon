...
Ponieważ problem w zasadzie jest jednowymiarowy, siatka została ustawiona jako 120 x 20, gdzie mamy 120 komórek w osi x. Kroki $dx$ i $dy$ odpowiadają 1 km w skali fizycznej. Aby symulacja była numerycznie stabilna, $dt$ musi zostać dobrane w taki sposób, żeby spełniony był warunek CFL, zatem żeby prędkość fali reprezentowana numerycznie poprzez liczbę Couranta nie przekraczała 1. W związku z tym dla problemu ustalono $dt/dx=dt/dy=0.005$. W przeliczeniu na jednostki fizyczne, $dt = 5\text{ s}$. Ilość kroków czasowych, przy której uzyskiwany wynik jest satysfakcjonujący to 10 000, więc 14 h. \\
...
Dla potwierdzenia poprawności uzyskiwanego wyniku numerycznego przeprowadzono porównanie wyniku numerycznego i przewidywanego wyniku na podstawie \eqref{eq:green}, które zostało przedstawione na rysunku \ref{fig:comparison}, gdzie $db/dx=0.0225$.
\begin{figure}[H]
    \centering
...
\end{figure}
Różnica między wynikiem numerycznym a analitycznym wynika z ograniczeń prawa Greena, które obowiązuje jedynie na łagodnie zmieniającym się dnie i dla fal o małej amplitudzie. W obszarze stromego spłycenia batymetrii założenia „mild-slope” przestają być spełnione, pojawia się częściowe odbicie energii oraz nieliniowe efekty związane z początkiem załamywania fali, których model analityczny nie uwzględnia. W efekcie prawo Greena przewiduje niefizycznie duży wzrost amplitudy, natomiast model numeryczny oparty na równaniach płytkowodnych daje realistycznie niższe maksimum. Następujący po maksimum nagły spadek i opadanie maksimum fali najprawdopodobniej związane jest z efektami numerycznymi.
...
\noindent
Dla lepszego zilustrowania porównania uzyskanych rezultatów, $\zeta_{max}$ dla obu siatek wraz z ich różnicą przedstawiono w tabeli \ref{tab:comparison}.
...
Na podstawie uzyskanych danych należy stwierdzić, że wyniki dla bardzo stromego wybrzeża nie mogą zostać użyte do wyciągnięcia wniosków ze względu na duży wpływ efektów numerycznych na wynik. Niezależnie od siatki kształt amplitudy fali staje się skokowy, co nie ma sensu fizycznego.
